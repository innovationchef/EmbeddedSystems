...
volatile uint8_t dataIn1[32]= {1,X,X,X,X,0,50,100,50,100,0,50,100,50,100,0,50,100,50,100,0,50,100,50,100,0};
volatile int velocity[4]={0}, dribbler=0;
volatile int bot_id;
int i=0;
int flag=0;
      ...
      if((EXTI->IMR & EXTI_IMR_MR13) && (EXTI->PR & EXTI_PR_PR13))
	    {
			...
			if(dataIn[0]==1)
			{
			  for(i=0;i<5;i++)
			  {
				if(bot_id==(i+1))
				{
					velocity[0] = dataIn[i+1];
					velocity[1] = dataIn[i+2];
					velocity[2] = dataIn[i+3];
					velocity[3] = dataIn[i+4];
					dribbler = dataIn[i+5];
				}
			  }
				velocity_set();
