			...
			if(dataIn[0]==1)
			{
			 for(i=0;i<5;i++)
			 {
				if(bot_id==(i+1))
				{
					...
					if(dataIn[1]==1)
					{
						GPIO_ResetBits(GPIOE, GPIO_Pin_1);   //RED
						GPIO_SetBits(GPIOE, GPIO_Pin_0); //GREEN
					}
					else {
						GPIO_ResetBits(GPIOE, GPIO_Pin_0);   //GREEN
						GPIO_SetBits(GPIOE, GPIO_Pin_1); //RED
					...
					for(j=0;j<5;j++)
						 {
							if(bot_id==(j+1))
							{
								velocity[0] = dataIn[j*5+1];
								velocity[1] = dataIn[j*5+2];
								velocity[2] = dataIn[j*5+3];
								velocity[3] = dataIn[j*5+4];
								dribbler = dataIn[j*5+5];
							}
